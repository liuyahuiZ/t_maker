...
class MyRecordsDoc extends Component {
    ...
    render() {
        ...
        const productListDom = productList&&productList.length > 0 ? productList.map((itm, idx)=>{
          return (<Row className="padding-all border-radius-2f padding-bottom-3 margin-bottom-1r bg-show cursor-pointer"  key={`${idx}-itm`}
        ...
        return(
          ...
          {isPC ? '': <Header
            ...
            <div className={`${isPC ? '': 'has-header '}`}>
              ...
              <Col span={12} className="line-height-3r font-size-12">{tagName||''} </Col>
