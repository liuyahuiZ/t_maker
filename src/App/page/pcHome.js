...
class Demo extends Component {
    ...
    render() {
        ...
        const tagTypeDom = typeArr&&typeArr.length > 0 ? typeArr.map((itm, idx)=>{
          return <Col key={`${idx}-type`} span={6} className="text-align-center padding-all cursor-pointer" onClick={()=>{
            console.log(itm, idx);
        ...
        const productListDom = productList&&productList.length > 0 ? productList.map((itm, idx)=>{
          let itemDom = itm.records&&itm.records.length > 0 ?  itm.records.map((em, ix)=>{
            return  <Col span={6} gutter={16} key={`${ix}-ems`} className={`line-height-2r margin-bottom-1r margin-right-3 cursor-pointer`} onClick={()=>{
        ...
        const hotDom = hotFiles&&hotFiles.length > 0 ? hotFiles.map((itm, idx)=>{
            return  <Col key={`${idx}-ems`} className={`line-height-2r cursor-pointer`} onClick={()=>{
      ...
      return(
          <section className="padding-all">
              <div className="heighr-13 border-radius-5f overflow-hide relative">
              <Carousel options={carouselMap} containerStyle={{borderRadius: '0.5rem', height:'13rem'}} dotDefaultStyle={{width: '5px'}} dotActiveStyle={{}} showDotsText={false} dragAble />
              </div>
              ...
              <Row  className="bg-show margin-bottom-1r margin-top-1r border-radius-2f">
                ...
                <Row><Col span={20}>热门下载</Col><Col span={4} className="text-align-right font-size-8 cursor-pointer"
                  onClick={()=>{ goLink('/HotRecords', {type: 'hot'})}}
              ...
              <Row  className="bg-show margin-bottom-1r margin-top-1r border-radius-2f">
                <Col className=" font-size-12 padding-all-2 border-bottom border-color-ddd">
                <Row><Col span={20}>最近更新</Col><Col span={4} className="text-align-right font-size-8 cursor-pointer"
                onClick={()=>{goLink('/HotRecords', {type: 'new'})}}
